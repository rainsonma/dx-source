...
import { AddVocabDialog } from "@/features/web/ai-custom-vocab/components/add-vocab-dialog";
import { SortableMetaItem } from "@/features/web/ai-custom-vocab/components/sortable-meta-item";
import { SortableContentItem } from "@/features/web/ai-custom-vocab/components/sortable-content-item";
import {
  reorderMetaAction,
...
} from "@/features/web/ai-custom-vocab/helpers/generate-items-api";
import type { LevelMeta, LevelContentItem } from "@/features/web/ai-custom-vocab/actions/course-game.action";
...
import { MAX_METAS_PER_LEVEL } from "@/features/web/ai-custom-vocab/helpers/format-metadata";
import { ProcessingOverlay } from "@/features/web/ai-custom-vocab/components/processing-overlay";
...
export function LevelUnitsPanel({
  ...
  const handleBreak = useCallback(async () => {
    ...
    const result = await breakVocabMetadata(levelId, controller.signal, (event) => {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    const result = await generateVocabContentItems(levelId, controller.signal, (event) => {
      setProgress({ done: event.done, total: event.total });
    });
              ...
              <div className="flex items-center gap-2 lg:flex-col lg:items-start xl:flex-row xl:items-center">
                <div className="flex items-center overflow-hidden rounded-lg bg-gradient-to-r from-teal-500 via-teal-600 to-teal-700">
                  <button
                    ...
                    title={readOnly ? "已发布的游戏不可编辑，请先撤回" : (isAtCapacity ? `已达上限（${metas.length}/${MAX_METAS_PER_LEVEL}）` : undefined)}
