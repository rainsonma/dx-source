...
import { replaceVocabAction } from "@/features/web/ai-custom/actions/content-vocab.action";
...
type EditVocabDialogProps = {
  vocab: ContentVocabData;
  onClose: () => void;
  onSaved: (updated: ContentVocabData) => void;
};

export function EditVocabDialog({ vocab, onClose, onSaved }: EditVocabDialogProps) {
  ...
  function handleSave() {
    ...
    if (!trimmedContent) {
      toast.error("词条内容不能为空");
      return;
    }

    const validDefs = posRows.filter((r) => r.gloss.trim().length > 0);
    const patch: ContentVocabReplacePatch = {
    ...
    startTransition(async () => {
      const res = await replaceVocabAction(vocab.id, patch);
      if (res.code !== 0) {
        if (res.code === 40301) {
          toast.error("无权完整编辑此词条");
        } else {
            ...
            <div className="flex justify-end">
              <div className="flex overflow-hidden rounded-xl border border-border">
                ...
                >
                  ...
                  <span className="text-sm font-semibold text-white">保存编辑</span>
                </button>
              </div>
            </div>
          </div>
